...
describe("filesize", () => {
  ...
  describe("Rounding", () => {
    ...
    it("should use different rounding methods", () => {
      assert.strictEqual(
        filesize(1536, { roundingMethod: "floor" }),
        "1.53 kB"
      );
      assert.strictEqual(filesize(1536, { roundingMethod: "ceil" }), "1.54 kB");
      assert.strictEqual(
        filesize(1536, { roundingMethod: "round" }),
        "1.54 kB"
      );
    });
  });
  ...
  describe("Custom symbols", () => {
    it("should use custom symbols when provided", () => {
      ...
      assert.strictEqual(
        filesize(1000, { symbols: customSymbols }),
        "1 kilobyte"
      );
    });
  });
  ...
  describe("Precision option", () => {
    ...
    it("should work with precision and different standards", () => {
      const largeNum = 1234567890;
      assert.strictEqual(
        filesize(largeNum, { precision: 2, standard: "iec" }),
        "1.1 GiB"
      );
      assert.strictEqual(
        filesize(largeNum, { precision: 2, standard: "jedec" }),
        "1.1 GB"
      );
      assert.strictEqual(
        filesize(largeNum, { precision: 2, standard: "si" }),
        "1.2 GB"
      );
    });
    ...
    it("should work with precision and bits option", () => {
      const largeNum = 1234567890;
      assert.strictEqual(
        filesize(largeNum, { precision: 2, bits: true }),
        "9.9 Gbit"
      );
      assert.strictEqual(
        filesize(largeNum, { precision: 3, bits: true }),
        "9.88 Gbit"
      );
    });
    ...
    it("should work with precision and object output", () => {
      ...
      assert.strictEqual(result.rawValue, 1.2);
  ...
  describe("Error handling", () => {
    ...
    it("should throw TypeError for invalid rounding method", () => {
      ...
      assert.throws(
        () => filesize(1024, { roundingMethod: "nonexistent" }),
        TypeError
      );
    });
  });
  ...
  describe("Input type handling", () => {
    ...
    describe("BigInt input", () => {
      ...
      describe("partial() with BigInt input", () => {
        it("should handle regular BigInt values", () => {
          const formatCustomSymbols = partial({
            symbols: { KiB: "kibibyte" },
            standard: "iec",
          });
